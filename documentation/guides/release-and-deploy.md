...
**Note🗒️** The following steps require admin access to the `Shopify/libra-web-tools` GitHub repo.
...
**Note🗒️** `libra-web-tools` packages adhere to [Semantic Versioning](http://semver.org/spec/v2.0.0.html).
...
- Log in to [Shipit](https://shipit.shopify.io/shopify/libra-web-tools/production)
...
- In your branch, run `yarn run release-beta`. Lerna will launch it's CLI to select a version for the changed packages. Select the `Custom` option and enter a version with an appended `-beta.X` (eg. `0.29.10-my-feature-beta.1`). Many `libra-web-tools` packages reference others. If your are prompted to version other packages, it is safe to do so.
